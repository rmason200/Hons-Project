...
# data send function (every second) - in: unit B status
# data read function (every 10 seconds) out: readings at A, readings at B
# cycle end function - in: sensor values, green split. out: periods a & b durations, start/end values

# section functions (start, period a, transition, period b, end)
...
def data_send(): # function to send data to API for Unit B
	...
	# * * * * * * * * * *
	# NON-ESSENTIAL: sends status of API for visualisation
	aio.send('status-a', sts_A)
	# * * * * * * * * * *
...
def data_read(): # function to take sensor inputs and store in list
	...
	# * * * * * * * * * * *
	# NON-ESSENTIAL: the following lines send the information for the values at A to the API
	# for visualisation only
	print("Sending sensor data to API...")
	aio.send('status-a', sts_A)
	aio.send('eco2-a', eco2_A)
	aio.send('tvoc-a', tvoc_A)
	aio.send('redu-a', redu_A)
	aio.send('oxi-a', oxi_A)
	# * * * * * * * * * * *
...
def period_a_sec(): # function to control signal status at each second of the cycle start section
	...
	# start timer to keep loop within a second
